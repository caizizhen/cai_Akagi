//! Tenpai-rate tables for opened-hand opponents.
//!
//! Source: `reference/mahjong-helper/util/tenpai_data.go`. Indexed by
//! `[melds 1..=3][turn 1..=18][tedashi-after-last-meld 0..]`. Values are
//! published statistical estimates for 4p mahjong.

/// Per-meld-count tenpai-rate sub-table. Each inner slice is variable-length —
/// the `turn` index walks one extra step per call to its prefix table; the
/// final dimension is always at most `turn + 1` long. Callers must clamp.
pub fn rate(melds: usize, turn: usize, tedashi: usize) -> f64 {
    let table: &[&[f64]] = match melds {
        1 => &TENPAI_RATE_1,
        2 => &TENPAI_RATE_2,
        3 => &TENPAI_RATE_3,
        _ => return 0.0,
    };
    let turn = turn.min(table.len() - 1);
    let row = table[turn];
    if row.is_empty() {
        return 0.0;
    }
    let t = tedashi.min(row.len() - 1);
    row[t]
}

/// Three-player approximation: derived from the four-player rate via the
/// `r * (2 - r/100)` saturation function in the Go reference.
pub fn rate_3p(rate_4p: f64) -> f64 {
    rate_4p * (2.0 - rate_4p / 100.0)
}

/// 1-meld tenpai rates `[turn][tedashi]`. Turn `t` is the t-th opponent
/// discard; index 0 is the pre-game placeholder.
const TENPAI_RATE_1: [&[f64]; 19] = [
    &[0.0],
    &[1.05],
    &[1.72, 3.37],
    &[2.90, 5.60, 8.53],
    &[5.26, 9.34, 13.45, 17.86],
    &[8.56, 14.42, 19.88, 25.19, 31.25],
    &[13.05, 21.04, 27.89, 33.85, 40.21, 47.34],
    &[18.34, 28.05, 35.71, 42.58, 48.71, 55.85, 63.21],
    &[24.38, 35.07, 43.76, 51.24, 57.73, 63.48, 68.56, 74.68],
    &[
        30.11, 41.25, 50.79, 58.60, 64.60, 70.18, 74.26, 78.25, 80.23,
    ],
    &[
        36.12, 47.47, 57.50, 64.90, 71.13, 76.06, 79.35, 82.00, 83.50, 83.50,
    ],
    &[
        41.39, 53.62, 62.82, 69.87, 75.56, 79.85, 83.07, 85.42, 87.92, 89.02,
    ],
    &[
        46.38, 58.24, 66.75, 74.18, 80.05, 84.12, 87.14, 89.50, 91.41, 92.73,
    ],
    &[
        49.21, 61.84, 70.55, 78.84, 82.92, 86.07, 88.99, 91.38, 93.44, 94.63, 96.42,
    ],
    &[
        52.07, 63.94, 73.27, 81.35, 85.42, 88.85, 91.24, 93.14, 94.28,
    ],
    &[
        56.89, 68.11, 75.01, 81.98, 84.55, 87.98, 90.60, 92.04, 93.63,
    ],
    &[62.86, 73.24, 79.46, 83.20, 86.56, 90.76, 92.81],
    &[67.42, 75.09, 80.07, 80.07, 80.07, 86.71],
    &[69.02, 84.51, 84.51],
];

/// 2-meld tenpai rates `[turn][tedashi]`.
const TENPAI_RATE_2: [&[f64]; 19] = [
    &[0.0],
    &[0.0],
    &[9.92],
    &[12.10, 17.72],
    &[17.54, 25.55, 32.77],
    &[23.24, 32.99, 41.82, 53.61],
    &[30.32, 41.77, 51.53, 62.77, 68.97],
    &[37.55, 49.50, 60.05, 69.07, 74.40, 76.53],
    &[43.52, 56.91, 67.17, 75.04, 79.34, 80.43, 88.26],
    &[50.28, 63.58, 73.48, 80.34, 83.85, 84.45, 89.63, 89.63],
    &[
        55.06, 69.27, 77.62, 84.93, 88.26, 89.04, 90.73, 93.82, 96.91,
    ],
    &[
        62.00, 74.63, 82.16, 88.18, 91.51, 92.42, 94.17, 97.08, 98.54, 98.54,
    ],
    &[66.04, 78.10, 85.15, 89.57, 93.22, 94.76, 95.51, 98.20],
    &[70.83, 81.37, 88.29, 91.52, 94.40, 96.40, 97.20, 98.60],
    &[72.52, 82.80, 89.17, 92.46, 94.97, 96.52, 96.52],
    &[75.00, 85.45, 90.74, 94.57, 95.85, 96.68, 96.68, 96.68],
    &[76.98, 85.15, 89.27, 93.44, 94.38, 95.18, 95.18],
    &[81.56, 88.65, 91.89, 96.76, 96.76],
    &[84.01],
];

/// 3-meld tenpai rates `[turn][tedashi]`.
const TENPAI_RATE_3: [&[f64]; 19] = [
    &[0.0],
    &[0.0],
    &[0.0],
    &[43.81],
    &[52.07, 63.48],
    &[61.44, 70.28, 87.26],
    &[68.20, 77.06, 90.17],
    &[72.56, 82.85, 88.56, 88.56],
    &[77.58, 87.59, 90.93],
    &[81.16, 90.22, 92.74, 97.58],
    &[84.53, 91.37, 94.75],
    &[86.23, 92.28, 96.31, 98.15],
    &[87.26, 93.05, 96.53, 98.26, 99.99],
    &[87.24, 94.11, 96.83],
    &[87.35, 94.58, 95.93],
    &[87.66, 93.15, 94.29],
    &[89.03, 91.77, 93.42, 93.42],
    &[89.54, 89.54],
    &[90.19],
];

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn one_meld_turn_5_tedashi_2() {
        // Anchor: 1 meld, turn 5, tedashi 2 → 19.88%.
        assert!((rate(1, 5, 2) - 19.88).abs() < 1e-6);
    }

    #[test]
    fn three_meld_turn_3() {
        assert!((rate(3, 3, 0) - 43.81).abs() < 1e-6);
    }

    #[test]
    fn out_of_range_clamps() {
        // Beyond turn-18: clamp.
        let v = rate(1, 100, 100);
        assert!(v > 0.0);
    }

    #[test]
    fn three_player_saturates_to_under_100() {
        let r4 = 90.0;
        let r3 = rate_3p(r4);
        assert!(r3 > r4 && r3 <= 100.0);
    }
}
